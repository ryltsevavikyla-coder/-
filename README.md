...
![1](https://github.com/ryltsevavikyla-coder/-/blob/main/Screenshot%202026-04-23%20145108.png)
...
echo "=== Резервное копирование домашней директории начато $(date) ==="

rsync -a --delete --checksum --exclude='.*' ~/ /tmp/backup/

if [ $? -eq 0 ]; then
    echo "✓ Резервное копирование успешно завершено $(date)"
    echo "Бэкап сохранён в /tmp/backup"
else
    echo "✗ ОШИБКА при выполнении резервного копирования $(date)"
fi

echo "============================================"
```

### Скриншот полностью 2 задания. резервоное копирование  будет каждый день в 7 утра

![2](https://github.com/ryltsevavikyla-coder/-/blob/main/Screenshot%202026-04-23%20150822.png)
